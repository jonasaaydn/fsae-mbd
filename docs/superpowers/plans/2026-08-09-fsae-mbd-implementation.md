...
> **記号表の申し送り（本計画の範囲外・章を書くときに追記する）**
> `notation.astro` は「章を書くたびに追記する」設計。現時点で意図的に未収録なのは次のカテゴリで、
> 該当章を書くときに必ず足すこと。抜けたまま章を書くと記号の定義が本文に散らばる。
> - サスペンション（12-14章）: トレッド幅、ロール角 φ、キャンバ角、トー角
> - 操作系（19-20章）: ハンドル角（実舵角 δ とは別物）、ラック比
> - ブレーキ（17-18章）: 制動力配分比、ブレーキ温度
> - エンジン・駆動系（21-22章）: エンジントルク、ギヤ比、最終減速比
> - 状態推定（34-35章）: 状態ベクトル、カルマンゲイン、共分散行列
> - 制御（36-39章）: PIDゲイン、LQRの重み行列 Q・R、フィードバックゲイン K
...
const ROUTES = [
  ...
  },
  {
    ...
    note: '大学のメールアドレスで MathWorks アカウントを作り、実際に「Get MATLAB」を試すと契約の有無がその場で分かります（大学名で検索できる公開一覧は存在しません）。分からなければ大学のIT部門か指導教員に確認してください。チーム申請より早い場合があります。',
  },
];
---
